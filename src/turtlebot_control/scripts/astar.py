...
class AStar(object):
    ...
    def get_neighbors(self, x):
        ...
        for v_idx in [-1,0,1]:
            for h_idx in [-1,0,1]:
                if v_idx != 0 or h_idx != 0: # can't stay put
                    # assuming square grid
                    dx = h_idx * self.resolution
                    dy = v_idx * self.resolution
                    xP = (x[0] + dx, x[1] + dy)
                    if self.is_free(xP):
                        neighbors.append(xP)
        return neighbors
    ...
    def solve(self):
        while len(self.open_set)>0:
            ...
            if x == self.x_goal:
                ...
                return True
            self.open_set.remove(x) # remove x from open set
            self.closed_set.append(x)
            for xP in self.get_neighbors(x):
                g = self.g_score[x] + self.distance(x,xP)
                if xP not in self.open_set and xP not in self.closed_set:
                    self.open_set.append(xP)
                    self.came_from[xP] = x
                    self.g_score[xP] = g
                    self.f_score[xP] = g + self.distance(xP,self.x_goal)
                elif g < self.g_score[xP]:
                    self.came_from[xP] = x
                    self.g_score[xP] = g
                    self.f_score[xP] = g + self.distance(xP,self.x_goal)
